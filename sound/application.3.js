...
soundManager.url = './soundmanagerv297a-20111220/swf/';
...
$(function(){
	...
	soundManager.onready(function() {
		...
		var consumer_key = "c94f375c2faed927b8971c5ce89c82d6",
				url = "http://soundcloud.com/miikesnow/devils-work"; //"http://soundcloud.com/foofighters/sets/wasting-light";		
		...
		//$.getJSON('http://api.soundcloud.com/resolve?url=' + url + '&format=json&consumer_key=' + consumer_key + '&callback=?', function(playlist){
                $.getJSON('./json/1.json', function(playlist){
			...
			$.each(playlist.tracks, function(index, track) {
				...
				// Create a list item for each track and associate the track *data* with it.
			        ...
			        $('<dt>' + '<img src="' + track.pic + '" alt="Miike Snow" height="200" width="200" />' + '</dt>').data('track', track).appendTo('.tracks');
		...
		$('.tracks dt').live('click', function(){
			...
			if (playing) {
				...
				if ($track.siblings('dt').hasClass('active')) { soundManager.stopAll(); }
			...
			$track.toggleClass('active').siblings('dt').removeClass('active');
		...
		$('.play, .pause').live('click', function(){
			...
			if ( $('dt').hasClass('active') == true ) {
				
				// If a track is active, play or pause it depending on current state.
				
				soundManager.togglePause( 'track_' + $('dt.active').data('track').id );	
		...
		var nextTrack = function(){
			...
			if ( $('dt.active').next().click().length == 0 ) {
				$('.tracks dt:first').click();
			}
		...
		var prevTrack = function(){
			...
			if ( $('dt.active').prev().click().length == 0 ) {
				$('.tracks dt:last').click();
			}
